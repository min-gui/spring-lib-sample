...
| `dev/deploy-YYYY-MM-DD`   | 주간 배포, 추후 자동 생성     | dev/main   | `feature/*` |
| `dev/main`                | 개발에서 검증된 코드         | main 기준    | `feature/*` |
| `stage/deploy-YYYY-MM-DD` | 주간 배포, 추후 자동 생성     | stage/main | `feature/*` |
...
- `{dev/stage}/deploy-YYYY-MM-DD` 브랜치가 존재하는 이유
...
flowchart TD
    ...
    H --> I{스테이지 배포?}
    I -->|예| J["feature/* → stage/deploy-YYYY-MM-DD 머지"]
    I -->|아니오| K[개발 완료]
    ...
    V[주간 검증된 코드] --> W["dev/deploy-YYYY-MM-DD 자동 생성"]
    V --> X["stage/deploy-YYYY-MM-DD 자동 생성"]
```